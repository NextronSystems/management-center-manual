...
.. list-table::
    ...
    * - Type
      - Description
    * - Bugfix
      - Resolved sporadic "unknown asset" error encountered during synchronization with Master Management Center
    * - Bugfix
    ...
    * - Bugfix
      - Addressed an issue where collected Bifrost samples could not be downloaded from the Master Management Center interface
    * - Bugfix
...
.. list-table::
    ...
    * - Bugfix
      - Removed the "Automatically accept all asset requests" setting from Master Management Center, as this configuration must be managed directly on the Management Center
    * - Bugfix
      - Fixed a bug causing cloned tasks on Master Management Center to remain in a perpetual pending state
    * - Bugfix
    ...
    * - Bugfix
      - Resolved inconsistencies in the statistics displayed on the Master Management Center overview page
    * - Bugfix
      - Fixed an issue where the "Stop" and "Interrupt" buttons did not function correctly for certain group scans on the Master Management Center
...
.. list-table::
    ...
    * - Type
      - Description
    * - Bugfix
      - Fixed a bug, which caused the maintenance task 'Move asset to another Management Center' to fail
    * - Bugfix
...
* Breaking Changes
  ...
  - The Endpoint Agent now also acts as the Service Controller. Existing Service Controller installations will be automatically put into sleep mode. Deployed Aurora Agents will still work as expected

----

* Highlights

  - The Endpoint Agent will from now on receive new tasks in real time.
  - The Endpoint Agent will from now on be able to run multiple tasks in parallel.
  - With the new real time agent, there will also be a new field 'Status' in the asset table (online/offline).
  - Added 'Essential Mode' to the Endpoint Agent, including only THOR and Aurora. It's ideal for critical systems where only the basic functionality is needed.
  - New 'Managed Service' mode that will use an Security Center to manage the asset's licenses on a per-tenant basis.
  - Integrated THOR Thunderstorm into the Management Center to scan samples sent from any device within the network.
  - New agent module 'File Browser' that allows you to browse the file system of the Endpoint Agent.

----

* Features

  - New agent module 'Sysstats' that allows you to view the system statistics such as CPU, RAM, and disk usage of the Endpoint Agent in real time.
...
* Improvements
  ...
  - Status of Master Management Center now represents the status of the connected Management Centers
  ...
  - Improved and migrated the Agent API validators from the Gatekeeper
...
* UX
  ...
  - Added Search Query to Service Control section
  ...
  - Added filters to the Endpoint Agent Installers page
  ...
  - Improved error messages when linking or synchronizing with Master Management Center or Analysis Cockpit fails
...
* Bugfixes
  ...
  - Fixed some non-working filters on the Master Management Center
  - Fixed description of allowed characters for Endpoint Agent Installer affix
  - When deleting IOCs, the affected rulesets have not been marked as 'uncompiled changes'
  - Fixed a race condition during synchronization with Analysis Cockpit
  - Fixed wrong dialog when disconnecting an Gatekeeper
  - Removed directories from the Installer page
  ...
  - Fixed Endpoint Agent Installer repacker to not touch the /usr/share directory
  - Fixed Endpoint Agent Installer repacker for AIX not working when also using agent obfuscation
...
* Chore
  ...
  - Wordings
  - Removed some deprecated playbooks like installation of the Service Controller or uninstalling the version 1 Agent
  - Removed obsolete 'fast poll' mode from the Endpoint Agent