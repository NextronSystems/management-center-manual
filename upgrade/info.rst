...
- **API Updates**: Various endpoints now use UUIDs instead of integer IDs.
  ...
  for details.
- **Configuration**: Migrated from key-value pairs to structured
  YAML format
...
- **License Management**: Unused THOR and Aurora endpoint licenses
  ...
  that the endpoint is not actively using any of the license features
...
- **THOR 11 Readiness**: Groundwork in place for the upcoming THOR 11
  Techpreview Release
...
- **Improved Update Version Control**: Better control over product versions
  with constraint-based pinning
...
- Will my existing configurations be preserved?
  ...
  - Yes, all configurations will be preserved during the upgrade. However,
    due to the completely reworked product updates and version pinning system,
    it's worth checking the version pinning configuration in the Updates
    settings after the upgrade to ensure the constraints and update intervals
    still fit your requirements.

- Are there breaking API changes in v4?

  - Yes, v4 introduces significant API changes. Many endpoints now use UUIDs
    instead of integer IDs, and product update management has been restructured
    to use version pinnings. Please review the
...
- Do I need to upgrade my agents to work with v4?
  ...
  - No, the upgrade does not require new agents. All currently deployed agents
    will continue to work seamlessly with v4 without any updates needed.
